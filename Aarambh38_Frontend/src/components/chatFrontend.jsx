...
useEffect(() => {
  ...
  axios
    .get(`${BASE_URL}/groupmessages/${selectedGroup._id}`, {
...
useEffect(() => {
  ...
  socketRef.current.on("groupMessageReceived", (msg) => {
...
const handleSendFinal = () => {
  ...
  if (selectedGroup) {
    ...
    handleSend(); // your existing private send
  }
...
useEffect(() => {
  ...
  axios
    .get(`${BASE_URL}/mygroups`, { withCredentials: true })
  ...
  <main className="pt-[64px] md:pt-[5px]">
    ...
    <>
      <button
        ...
        className="mx-4 my-2 px-3 py-2 bg-blue-600 text-white rounded w-[calc(100%-2rem)]"
      >
        + Create Group
      </button>

      <div className="px-4 mt-3 font-semibold text-gray-500">
        Groups
      </div>
      ...
      >
        <div
          className={`inline-block px-4 py-2 rounded-lg text-white text-sm ${
            m.senderId === user._id ? "bg-green-600" : "bg-blue-600"
          }`}
        >
          <div className="text-xs opacity-70 mb-1">
            {m.senderId === user._id ? "You" : "Member"}
          </div>
          {m.text}
        </div>
    ...
    <>
      {Object.entries(groupedMessages).map(([dateLabel, group]) => (
        <div key={dateLabel}>
          ...
          {group.map((msg) => (
            /* ⚠️ YAHAN TUMHARA POORA EXISTING MESSAGE JSX AAYEGA */
            /* (jo tumne already likha hua hai — SAME AS IT IS) */
