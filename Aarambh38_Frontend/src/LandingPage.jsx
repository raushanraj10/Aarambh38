...
export default function StudentLandingPage() {
  ...
  const [loading, setLoading] = useState(true);
  const [alumniList, setAlumniList] = useState([]);
  const [groupMode, setGroupMode] = useState("all"); // "all" | "college"
  ...
  const toggleCollege = (college) => {
    setExpandedCollege((prev) => (prev === college ? null : college));
  };

  // Group alumni by college
  const groupedByCollege = alumniList.reduce((acc, alumni) => {
    const college = alumni.collegeName || "Unknown College";
    if (!acc[college]) acc[college] = [];
    acc[college].push(alumni);
    return acc;
  }, {});
...
<label className="flex items-center gap-2 text-sm w-full">
  <input
    type="checkbox"
    checked={groupMode === "college"}
    onChange={() => setGroupMode("college")}
    className="w-4 h-4 flex-shrink-0"
  />
  
  {/* Show text on desktop, icon on mobile */}
  <span className="hidden sm:inline">College Wise</span>
  <GraduationCap className="w-4 h-4 sm:hidden text-blue-600" />
</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <button
              onClick={() => {
                setShowFilters(false);
                setFilters({ college: "", company: "", role: "", fullName: "", about: "" });
              }}
              className="px-4 py-2 bg-gray-300 text-gray-800 rounded-xl hover:bg-gray-400"
            >
              Back
          ...
          filteredAlumni.length === 0 ? (
            ...
            filteredAlumni.map((alumni) => (
              <AlumniCard
                ...
                loadingAlumniId={loadingAlumniId}
              />
            ))
          )
        ) : (
          // College wise
          Object.keys(groupedByCollege).map((college) => (
            <div key={college} className="w-full max-w-4xl bg-white rounded-xl shadow-md border">
              <button
                onClick={() => toggleCollege(college)}
                className="w-full text-left px-4 py-3 font-semibold text-blue-700 bg-gray-100 rounded-t-xl flex justify-between items-center"
              >
                {college}
                <span>{expandedCollege === college ? "▲" : "▼"}</span>
              </button>

              {expandedCollege === college && (
                <div className="p-6 flex flex-col gap-6">
                  {groupedByCollege[college].map((alumni) => (
                    <AlumniCard
                      key={alumni._id}
                      alumni={alumni}
                      requestStatus={requestStatus}
...
// 🔹 Reusable Alumni Card (keeps your original send request + chat UI)
function AlumniCard({
  alumni,
  requestStatus,
  acceptedStatus,
  ...
  return (
    <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-md border transform transition-all duration-300 ease-in-out hover:shadow-xl hover:scale-[1.015]">
      {isLoading && (
        <div className="absolute inset-0 bg-white bg-opacity-60 rounded-2xl flex items-center justify-center z-10">
          <div className="animate-spin w-8 h-8 border-4 border-blue-400 border-t-transparent rounded-full"></div>
        </div>
      ...
      <Link to={`/alumni/${id}`} className="flex flex-col sm:flex-row gap-6 p-6">
        <div className="shrink-0">
          <img
            src={
              alumni.photourl ||
              "https://media.istockphoto.com/id/1127115457/photo/black-hat-of-the-graduates-floating-in-the-sky.jpg"
            }
            alt="alumni"
            className="w-24 h-24 sm:w-28 sm:h-28 rounded-full object-cover border-2 border-blue-500 transition-transform duration-300 hover:scale-105"
          />
      ...
      <div className="px-6 pb-4">
        ...
        <div className="flex flex-wrap gap-4 pt-3">
          {!isAccepted && (
            ...
            >
              {isRequestSent ? "Request Sent" : isLoading ? "Sending..." : "Send Request"}
            </button>
          )}
