...
export default function AlumniBlocked() {
  ...
  const filteredStudents = blockedStudents.filter((req) => {
    const user = req.fromuserId;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-red-50 to-red-100 px-6 py-12">
      ...
      <div className="max-w-md mx-auto mb-10">
        <input
          type="text"
          placeholder="Search by name, batch, college, or branch..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full px-4 py-2 border rounded-xl shadow-sm focus:ring-2 focus:ring-red-400 focus:outline-none"
        />
      </div>

      {filteredStudents.length === 0 ? (
        <p className="text-center text-gray-500">No matching blacklisted students found.</p>
      ) : (
        <div className="max-w-6xl mx-auto flex flex-col gap-10">
